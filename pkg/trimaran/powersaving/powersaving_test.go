...
func TestNew(t *testing.T) {
	powerSavingArgs := pluginConfig.PowerSavingArgs{
		TrimaranSpec:     pluginConfig.TrimaranSpec{WatcherAddress: "http://deadbeef:2020"},
		LowCPUThreshold:  v1beta2.DefaultLowCPUThreshold,
		HighCPUThreshold: v1beta2.DefaultHighCPUThreshold,
	}
	powerSavingConfig := config.PluginConfig{
		Name: Name,
		Args: &powerSavingArgs,
	}
	...
	fh, err := testutil.NewFramework(registeredPlugins, []config.PluginConfig{powerSavingConfig},
	...
	assert.Nil(t, err)
	p, err := New(&powerSavingArgs, fh)
	assert.NotNil(t, p)
	assert.Nil(t, err)
}

func TestPowerSavingScoring(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
					...
					NodeMetricsMap: map[string]watcher.NodeMetrics{
						"node-1": {
							Metrics: []watcher.Metric{
								{
									Type:     watcher.CPU,
									Value:    float64(v1beta2.DefaultHighCPUThreshold + 10),
			...
			expected: []framework.NodeScore{
				{Name: "node-1", Score: v1beta2.DefaultHighCPUThreshold + 10},
			},
		},
		{
			test: "hot node",
					...
					NodeMetricsMap: map[string]watcher.NodeMetrics{
						"node-1": {
							Metrics: []watcher.Metric{
								{
									Type:     watcher.CPU,
									Value:    float64(v1beta2.DefaultLowCPUThreshold - 5),
									Operator: watcher.Latest,
								},
							},
						},
			...
			expected: []framework.NodeScore{
				{Name: "node-1", Score: v1beta2.DefaultHighCPUThreshold - 5},
			},
		},
		{
...
func BenchmarkPowerSavingPlugin(b *testing.B) {
	tests := []struct {
		name     string
		podsNum  int64
		nodesNum int64
		...
		b.Run(tt.name, func(b *testing.B) {
			...
			powerSavingArgs := pluginConfig.PowerSavingArgs{
				TrimaranSpec: pluginConfig.TrimaranSpec{
					MetricProvider: pluginConfig.MetricProviderSpec{Type: "Prometheus", Address: server.URL, InsecureSkipVerify: true},
